...
Uniti::Object &WaveSpawn::getObject() {
    return this->_object;
}
...
void WaveSpawn::update() {
    if (this->_clock.getSeconds() < this->_spawnTime) return;
    this->_clock.restart();
    for (auto &enemy : this->_enemies) {
        if (std::rand() % 100 > enemy.get("random", 5).asInt()) continue;
        auto object = std::make_unique<Uniti::Object>(enemy, this->_object.getScene());
...
const Uniti::Clock &WaveSpawn::getClock() const {
    return this->_clock;
}

Uniti::Clock &WaveSpawn::getClock() {
    return this->_clock;
}

const Uniti::Event &WaveSpawn::getEvent() const {
    return this->_event;
}

Uniti::Event &WaveSpawn::getEvent() {
    return this->_event;
}

extern "C" {
void *getCreator(void) {
    return new Uniti::PluginCreator<Uniti::IObjectPlugin, WaveSpawn, Uniti::Object>();
}

void deleteCreator(void *creator) {
    delete static_cast<Uniti::PluginCreator<Uniti::IObjectPlugin, WaveSpawn, Uniti::Object> *>(creator);
}
}
