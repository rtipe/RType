...
Entity::Entity(Uniti::Object &object) : _object(object) {
  ...
  this->_event.addEvent("basicAttack", [&](const Json::Value &value) {
    if (this->_attacks[0].clock.getSeconds() < this->_attacks[0].cooldown)
      return;
    this->_attacks[0].clock.restart();
    auto projectile = std::make_unique<Uniti::Object>(
        this->_attacks[0].projectileObject, this->_object.getScene());
    projectile.get()->getTransform().getPosition() =
        this->_object.getTransform().getPosition() + this->_spawnProjectile;
    this->_object.getScene().getObjects().add(std::move(projectile));
  });
  this->_event.addEvent("directionalAttack", [&](const Json::Value &value) {
    if (this->_attacks[1].clock.getSeconds() < this->_attacks[1].cooldown)
      return;
    this->_attacks[1].clock.restart();
    auto projectile = std::make_unique<Uniti::Object>(
        this->_attacks[1].projectileObject, this->_object.getScene());
    projectile.get()->getTransform().getPosition() =
        this->_object.getTransform().getPosition() + this->_spawnProjectile;
    this->_object.getScene().getObjects().add(std::move(projectile));
  });
  ...
  this->_event.addEvent("specialAttack", [&](const Json::Value &value) {
    if (this->_attacks[2].clock.getSeconds() < this->_attacks[2].cooldown)
      return;
    this->_attacks[2].clock.restart();
    auto projectile = std::make_unique<Uniti::Object>(
        this->_attacks[2].projectileObject, this->_object.getScene());
    projectile.get()->getTransform().getPosition() =
        this->_object.getTransform().getPosition() + this->_spawnProjectile;
    this->_object.getScene().getObjects().add(std::move(projectile));
  });
...
void Entity::awake(const Json::Value &value) {
  ...
  if (!this->_player) {
    this->_object.getName() =
        this->_object.getName() + "_" + std::to_string(Entity::_id);
    Entity::_id++;
  }
}
...
void Entity::update() {
  ...
  for (auto &attack : this->_attacks) {
    if (attack.clock.getSeconds() < attack.cooldown)
      continue;
    attack.clock.restart();
    auto projectile = std::make_unique<Uniti::Object>(attack.projectileObject,
...
void Entity::postUpdate() {
  ...
  if ((!this->_player &&
       this->_object.getTransform().getPosition().getX() < -100) ||
      this->_hp <= 0) {
    Json::Value data;
    data["name"] = "DestroyEntity";
    data["value"] = this->_object.getName();
...
const Uniti::Clock &Entity::getClock() const { return this->_clock; }

Uniti::Clock &Entity::getClock() { return this->_clock; }

const Uniti::Event &Entity::getEvent() const { return this->_event; }

Uniti::Event &Entity::getEvent() { return this->_event; }

Entity::EntityType Entity::getType() const { return this->_type; }

float &Entity::getHP() { return this->_hp; }
...
Entity::Attack::Attack(const Json::Value &attack) {
  ...
  this->projectileObject =
      Uniti::Object::openJsonFile(attack.get("projectile", "").asString());
}
