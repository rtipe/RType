...
UserController::UserController(Uniti::Object &object): _object(object) {
    this->_event.addEvent("playerMovement", [&] (const Json::Value &value) {
        ...
        auto object = Uniti::Object::find_if(this->_object.getCore(), [&] (const Uniti::Object &compare) {
    ...
    this->_event.addEvent("createPlayer", [&] (const Json::Value &value) {
        ...
        if (!this->_pathCharacter.contains(type)) return;
        auto object = std::make_unique<Uniti::Object>(this->_pathCharacter[type], this->_object.getScene());
    ...
    this->_event.addEvent("disconnectUser", [&] (const Json::Value &value) {
        ...
        this->_object.getScene().getObjects().remove(object.value().get().getName());
    });
...
Uniti::Object &UserController::getObject() {
    return this->_object;
}

void UserController::awake(const Json::Value &value) {
    for (const auto &character : value["characters"].getMemberNames())
        this->_pathCharacter[character] = Uniti::Object::openJsonFile(value["characters"][character].asString());
}
...
const Uniti::Clock &UserController::getClock() const {
    return this->_clock;
}

Uniti::Clock &UserController::getClock() {
    return this->_clock;
}

const Uniti::Event &UserController::getEvent() const {
    return this->_event;
}

Uniti::Event &UserController::getEvent() {
    return this->_event;
}

extern "C" {
void *getCreator(void) {
    return new Uniti::PluginCreator<Uniti::IObjectPlugin, UserController, Uniti::Object>();
}

void deleteCreator(void *creator) {
    delete static_cast<Uniti::PluginCreator<Uniti::IObjectPlugin, UserController, Uniti::Object> *>(creator);
}
}
