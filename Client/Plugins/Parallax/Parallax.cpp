...
Uniti::Object &Parallax::getObject() { return this->_object; }
...
void Parallax::postUpdate() {
  ...
  for (auto &object : this->_object.getChildren().getObjects()) {
    ...
    if (object->getTransform().getPosition().getX() <
      ...
      object->getTransform().getPosition().getX() =
          this->_width + (this->_end - exceeded - 2);
    }
  }
  this->_clock.restart();
}
...
const Uniti::Clock &Parallax::getClock() const { return this->_clock; }

Uniti::Clock &Parallax::getClock() { return this->_clock; }

const Uniti::Event &Parallax::getEvent() const { return this->_event; }

Uniti::Event &Parallax::getEvent() { return this->_event; }

extern "C" {
void *getCreator(void) {
  return new Uniti::PluginCreator<Uniti::IObjectPlugin, Parallax,
                                  Uniti::Object>();
}

void deleteCreator(void *creator) {
  delete static_cast<
      Uniti::PluginCreator<Uniti::IObjectPlugin, Parallax, Uniti::Object> *>(
      creator);
}
}
